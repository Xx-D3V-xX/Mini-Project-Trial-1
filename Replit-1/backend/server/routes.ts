...
import { itinerariesRepo } from "./db/repositories/itineraries";
import { visitHistoryRepo } from "./db/repositories/visitHistory";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/pois", async (_req: Request, res: Response) => {
    try {
      const pois = await poisRepo.getAllPOIs();
      res.json(pois);
    } catch (error: any) {
  ...
  app.get("/api/pois/:id", async (req: Request, res: Response) => {
    try {
      const poi = await poisRepo.getPOIById(req.params.id);
      if (!poi) {
  ...
  app.get("/api/profile", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const itineraries = await itinerariesRepo.getItinerariesByUser(req.userId!);
      const visitHistory = await visitHistoryRepo.getVisitHistory(req.userId!);
  ...
  app.get("/api/profile/itineraries", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const formattedItineraries = itineraries.map((itinerary) => ({
        ...
        days: itinerary.days,
        locations: (itinerary.data as any[]).reduce(
  ...
  app.get("/api/profile/history", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      const history = await visitHistoryRepo.getVisitHistory(req.userId!);
      const enrichedHistory = await Promise.all(
        history.map(async (visit) => {
          const poi = await poisRepo.getPOIById(visit.poiId);
          return {
            id: visit.id,
  ...
  app.post("/api/profile/itineraries", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const itinerary = await itinerariesRepo.createItinerary({
        userId: req.userId!,
        title,
        days,
        data,
  ...
  app.delete("/api/profile/itineraries/:id", authenticateToken, async (req: AuthRequest, res: Response) => {
    try {
      const itinerary = await itinerariesRepo.getItinerary(req.params.id);
      if (!itinerary) {
      ...
      await itinerariesRepo.deleteItinerary(req.params.id);
